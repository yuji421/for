...
function launchConfetti(durationMs = 1800){
  ...
  const pieces = [];
  const count = Math.min(180, Math.floor(W * 0.25)); // 画面幅に応じて数を調整
  ...
  for(let i=0; i<count; i++){
    pieces.push({
      ...
      // ピンク〜白系（作品の雰囲気に寄せる）
      color: (Math.random() < 0.6) ? "#ff5aa5"
            : (Math.random() < 0.75) ? "#ffd1e6"
            : (Math.random() < 0.9) ? "#fff3f9"
            : "#ffffff",
      life: 1
    });
  }
  ...
  function tick(now){
    ...
    // durationを超えたらフェードアウト
    const done = t >= durationMs || pieces.every(p => p.life <= 0);
    if(!done){
